...
const CompanySignupForm = () => { // Renamed component to CompanySignupForm
    ...
    const handleSubmit = async (event) => {
        ...
        const companyData = { // Renamed to companyData
        ...
        try {
            const response = await axios.post('http://localhost:5000/company/signup', companyData); // Updated API endpoint to /company/signup
    ...
    return (
        <div className="xl:-mt-20 flex-1 p-5 md:p-5 bg-transparent flex flex-col justify-center min-h-screen overflow-hidden">
            <div className="border border-gray-700 p-6 rounded-lg max-w-2xl mx-auto bg-[#1a1a24]">
                <h2 className="text-3xl font-light mb-6 text-center">Let's get your company hiring!</h2> {/* Updated Heading */}
                <form className="space-y-6" onSubmit={handleSubmit}>
                    <div>
                        <label className="block text-gray-400 font-light">Company Name*</label> {/* Updated Label */}
                        <input
                            type="text"
                    ...
                    <Link to="/company/login"> {/* Updated Link to Company Login */}
                        <button className="text-purple-500 hover:underline">Already have a company account? Login</button> {/* Updated Text */}
                    </Link>
